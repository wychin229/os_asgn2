...
Algorithms implemented : SRTN, quantum = 3
*/
#include "pcb.h"
#include "process.h"
#include <stdio.h>
...
/* Function to get shortest remaining PCB */
int get_shortest(pcb_t *pcb_arr, int rear)
{
	...
	for(int i=0; i<rear; i++){
		if(pcb_arr[i].remainingTime > 0) {
			if(smallest_time == 0) {
				smallest_time = pcb_arr[i].remainingTime;
				index = i;
			}
			else{
				if(pcb_arr[i].remainingTime < smallest_time){
					smallest_time = pcb_arr[i].remainingTime;
					index = i;
				}
			}
...
int main(int argc, char *argv[])
{
	...
	// simulate the scheduling - SRTF, Q=3
	int clock = 0;
	const int QUANTUM = 3; // time quantum
	int start_time = 0; // the clock time when a process get cpu 
	...
	while((has_item(p_front,p_rear)) == 0 || (has_unfinished(pcb_arr,pcb_rear)) != -1){
		...
		if(clock-start_time <= QUANTUM) {
			if(pcb_arr[run_int].remainingTime == pcb_arr[run_int].serviceTime || clock-start_time == 0) {
				if(pcb_arr[run_int].getcpu == -1){
			...
			else if(clock-start_time == QUANTUM && pcb_arr[run_int].remainingTime > 0) {
